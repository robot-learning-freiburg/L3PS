...
class ToTensor:
    def __call__(self, data: Dict[str, Any]) -> Dict[str, Any]:
        ...
        if "depth" in data:
            data["depth"] = TF.to_tensor(data["depth"])
        return data
    ...
    def __call__(self, data: Dict[str, Any]) -> Dict[str, Any]:
        ...
        if "center" in data:
            ...
            data["center"] = TF.resize(data["center"], self.size,
                                       interpolation=InterpolationMode.NEAREST)
        if "depth" in data:
            data["depth"] = TF.resize(data["depth"], self.size,
...
class MaskPostProcess:
    def __call__(self, data: Dict[str, Any]) -> Dict[str, Any]:
        ...
        if "depth" in data and len(data["depth"].shape) == 3:
...
class CopyPaste:
    ...
    def __call__(self, data: Dict[str, Any]) -> Dict[str, Any]:
        ...
        source_instance_ids = source_instance_ids[source_instance_ids != 0]  # remove background2345
