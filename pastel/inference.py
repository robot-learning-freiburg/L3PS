...
class PanopticCluster(pl.LightningModule):
    ...
    def __init__(self, semantic_model: pl.LightningModule, semantic_model_ckpt: str,
                 ...
                 output_size: Tuple[int, int],
                 upsample_factor_affinity_map: Optional[float],
                 neighbor_radius_affinity_matrix: Optional[int],
        ...
        ########### HACK ############# FOR DEBUGGING
        # semantic_model_ckpt = os.path.join('panoptic_label_generator', semantic_model_ckpt)
        # boundary_model_ckpt = os.path.join('panoptic_label_generator', boundary_model_ckpt)
        ################################################
    ...
    def add_instance(self, pred_sem_i, semantic_segment_mask,
                     mask: np.array, pred_instances, number_of_instances: int,
    ...
    def predict(self, rgb: torch.Tensor, rgb_original: torch.Tensor,
        ...
        pred_sem, pred_sem_prob = self.semantic_model.predict(rgb, ego_car_mask, depth_map)  # (B, H, W)
        device = pred_sem.device
        pred_sem = pred_sem.cpu().numpy()  # (B, H, W)
        pred_sem_prob = pred_sem_prob.cpu().numpy()  # (B, N_classses, H, W)
        ...
        if self.do_post_processing:
            ...
            pred_sem_prob[:, :, :self.boundary_margin, :] = self.ignore_index
            pred_sem_prob[:, :, -self.boundary_margin:, :] = self.ignore_index
            pred_sem_prob[:, :, :, :self.boundary_margin] = self.ignore_index
            pred_sem_prob[:, :, :, -self.boundary_margin:] = self.ignore_index
        ...
        if self.mode == 'ncut':
            ...
            affinity_map_size = [int(rgb_h // self.boundary_model.patch_size * \
                self.upsample_factor_affinity_map),
                                 int(rgb_w // self.boundary_model.patch_size * \
                                     self.upsample_factor_affinity_map)]

        pred_instances_batch = []
        pred_sem_batch = []
        pred_sem_prob_batch = []
        for rgb_i, rgb_original_i, pred_sem_i, pred_boundary_i, ego_car_mask_i, pred_sem_prob_i in \
                zip(rgb, rgb_original, pred_sem, pred_boundary, ego_car_mask, pred_sem_prob):
            ...
            if self.mode == 'ncut':
                ...
                affinity_matrix = compute_affinity_matrix(pred_boundary_i_downsampled,
                                                          beta=self.beta,
                                                          neighbor_radius=\
                                                              self.neighbor_radius_affinity_matrix)
            ...
            if self.do_post_processing:
                ...
                for s_id, s_size in zip(segment_id, segment_size):
                    ...
                    if s_id not in [0, 1] and s_size < pred_boundary_i.size / 8:
                        ...
                        semantic_classes = (semantic_classes[0][not_ignore_index],
                                            semantic_classes[1][not_ignore_index])
                        if len(semantic_classes[0]) > 1:
                            max_class = semantic_classes[0][np.argmax(semantic_classes[1])]
                            mask = np.logical_and(pred_boundary_segments == s_id,
                                                  pred_boundary_i == 1)
                            pred_sem_i[mask] = max_class
                            pred_sem_prob_i[max_class, mask] = 1.0
                            pred_sem_prob_i[~max_class, mask] = 0.0
                ...
                if self.do_post_processing:
                    ...
                    for s_id in np.unique(semantic_segments_mask):
                        ...
                        if len(semantic_classes) == 1 and semantic_classes[0] in thing_classes:
                            ...
                            pred_sem_prob_i[:, segment_mask] = self.ignore_index
                ...
                for semantic_segment_id in np.unique(semantic_segments_mask):
                    ...
                    if self.mode == 'ncut':
                        ...
                        semantic_segment_mask_downsampled = torch.nn.functional.interpolate(
                            torch.from_numpy(
                                semantic_segment_mask.astype(np.float32)
                        ...
                        while len(ncut_queue) > 0:  # iterative Ncut
                            ...
                            if np.sum(mask) < (self.instance_min_pixel[semantic_class_id] * \
                                (self.upsample_factor_affinity_map / \
                                    self.semantic_model.encoder.patch_size)**2):
                                instances_mask, pred_sem_i, semantic_segment_mask,\
                                    number_of_instances_in_segment = \
                                        self.add_instance(pred_sem_i,
                                                          semantic_segment_mask,
                                                          mask,
                                                          instances_mask,
                                                          number_of_instances_in_segment,
                            ...
                            affinity_matrix_semantic_segment = affinity_matrix[mask.reshape(-1),
                                                                               :]  # (K, W)
                            ...
                            with cp.cuda.Device(device.index):
                                ...
                                eigenvalues, eigen_maps = spectral_embedding(
                            ...
                            eigen_vec_hist, eigen_vec_bins = np.histogram(
                                eigen_vec[np.isfinite(eigen_vec)],
                                bins=self.eigen_vec_hist_bins)
                            eigen_vec_hist_ratio = np.min(eigen_vec_hist) / np.max(eigen_vec_hist)

                            # Do not cut if the cut cost is too high
                            if cut_cost > self.ncut_threshold or \
                                eigen_vec_hist_ratio > self.eigen_vec_hist_ratio:
                                instances_mask, pred_sem_i, semantic_segment_mask,\
                                    number_of_instances_in_segment = self.add_instance(
                                        pred_sem_i,
                                        semantic_segment_mask,
                                        mask,
                                        instances_mask,
                            ...
                            eigen_vec_thresholds = np.linspace(np.min(eigen_vec),
                                                               np.max(eigen_vec),
                        ...
                        pred_sem_prob_i[:, np.logical_and(semantic_segment_mask_original,
                                      np.logical_not(instances_mask > 0))] = self.ignore_index
                        ...
                        instances_mask = \
                            scipy.ndimage.label(instances_mask,
                                                structure=self.structure_connectivity)[
                                0]  # (H, W)
                        ...
                        if len(segment_id) > 1:
                            ...
                            for s_id, s_size in zip(segment_id, segment_size):
                                ...
                                if s_size < instance_min_pixel:
                                    ...
                                    pred_sem_prob_i[:, instances_mask == s_id] = self.ignore_index
            ...
            if self.do_post_processing:
                ...
                for semantic_class_id in stuff_classes:
                    ...
                    # Remove small instances from the semantic and the instance masks
                    segment_id, segment_size = np.unique(semantic_segments_mask,
                                                         return_counts=True)
                    for s_id, s_size in zip(segment_id, segment_size):
                        if s_id == 0:  # skip background
                            continue
                    ...
                    for s_id in np.unique(semantic_segments_mask):
                        ...
                        if np.sum(segment_mask * border_mask) > 0 and \
                            semantic_class_id in background_classes:
                            continue
            ...
            if len(coordinates_unknown) > 0:
                ...
                pred_sem_prob_i[predict_data[0], unknown_mask] = 1.0
                pred_sem_prob_i[~predict_data[0], unknown_mask] = 0.0
            ...
            pred_sem_prob_batch.append(pred_sem_prob_i)

        pred_instances = np.stack(pred_instances_batch, axis=0)  # (B, H, W)
        pred_sem = np.stack(pred_sem_batch, axis=0)  # (B, H, W)
        pred_sem_prob = np.stack(pred_sem_prob_batch, axis=0)  # (B, N_classes, H, W)

        return pred_instances, pred_sem, pred_boundary, pred_sem_prob
    ...
    def test_step(self, batch: Dict[str, Any], batch_idx: int, dataloader_idx: int = 0):
        if self.flag:
            self.trainer.progress_bar_callback._test_progress_bar.smoothing = 1
            self.flag = False
        # Get the lidar
        ...
        depth_map = batch.get('depth', None)  # (B, H, W)
        depth_map = None
        # Skip if mask already exists
        if self.test_save_dir is not None:
            image_path_i = batch['filename'][0]
            dataset_path_base = str(self.get_dataset().path_base)
            image_path_i = image_path_i.replace(dataset_path_base, self.test_save_dir)
            image_path_i = image_path_i.replace('.jpg', '.npy')
            sem_path_i = image_path_i.replace('samples', 'semantic_prediction')
            inst_path_i = image_path_i.replace('samples', 'panoptic_prediction')
            if os.path.exists(sem_path_i) and os.path.exists(inst_path_i):
                return
            os.makedirs(os.path.dirname(image_path_i), exist_ok=True)
            open(image_path_i, 'w').close()  # Dummy file to indicate that the file is being prepared

        pred_instances, pred_sem, pred_boundary, pred_sem_prob = self.predict(
            rgb, rgb_original, ego_car_mask, depth_map
        )
        ...
        if self.test_save_dir is not None:
            image_path = batch['filename']
            images = batch['rgb_original']
            dataset = self.get_dataset()
            dataset_path_base = str(dataset.path_base)

            for pred_sem_prob_i, pred_sem_i, pred_instances_i, pred_boundary_i, image_i, image_path_i in \
                    zip(pred_sem_prob, pred_sem, pred_instances, pred_boundary, images, image_path):

                pred_sem_i_gt_format, pred_panoptic_i_gt_format = \
                    dataset.compute_panoptic_label_in_gt_format(pred_sem_i, pred_instances_i)

                image_path_i = image_path_i.replace(
                    dataset_path_base, self.test_save_dir
                )
                pred_sem_i_path = image_path_i.replace(
                    'samples', 'semantic_prediction'
                ).replace('.jpg', '.npy')
                pred_panop_i_path = image_path_i.replace(
                    'samples', 'panoptic_prediction'
                ).replace('.jpg', '.npy')

                pred_sem_prob_i_path = pred_sem_i_path.replace('semantic_prediction', 'semantic_prob')

                os.makedirs(os.path.dirname(pred_sem_i_path), exist_ok=True)
                os.makedirs(os.path.dirname(pred_panop_i_path), exist_ok=True)
                os.makedirs(os.path.dirname(pred_sem_prob_i_path), exist_ok=True)
                np.save(pred_sem_i_path, pred_sem_i_gt_format)
                np.save(pred_panop_i_path, pred_panoptic_i_gt_format)
                np.save(pred_sem_prob_i_path, pred_sem_prob_i)


class PanopticClusterCLI(LightningCLI):

    def __init__(self):
        super().__init__(
            model_class=PanopticCluster,
