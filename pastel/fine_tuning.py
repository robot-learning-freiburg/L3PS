...
from models.dino_v1 import dino_vitb16
...
class FineTuner(pl.LightningModule):
    def __init__(self, vit_model: str, num_classes: int, blocks: Optional[List[int]] = None,
        ...
        if vit_model == 'vits14':
            ...
            self.encoder = dinov2_vitg14(pretrained=True, window_block_indexes=window_block_indexes,
                                         window_size=window_size)
            self.pretraining = 'dinov2'
        elif vit_model == 'dinov1_vitb16':
            self.encoder = dino_vitb16(pretrained=True)
            self.pretraining = 'dinov1'
        else:
    ...
    def forward_encoder_dinov2(self, img: torch.Tensor, feature_key: str = 'x'):
        ...
        return_attention_features = any([(feature_key in x)
                                         for x in ['q', 'k', 'v', 'attn']])
        with torch.no_grad():
    ...
    def multi_scale_test_augmentation(self, rgb: torch.Tensor, apply_softmax=False,
        ...
        for scale, stride_divider in zip(self.test_multi_scales,
                                         self.test_multi_scales_stride_divider):
            image_h_split, image_w_split = ceil(rgb_h / scale), ceil(rgb_w / scale)
            stride_h, stride_w = ceil(image_h_split / stride_divider), \
                ceil(image_w_split / stride_divider)
            ...
            # (B, C * image_h_split * image_w_split, num_patches)
            ...
            for patch_i in range(patches.shape[-1]):
                ...
                if boundary_margin is not None:  # Note: only works for boundary estimation
                    ...
                    pred_i[:, :, :, :boundary_margin] = 1
                    pred_i[:, :, :, -boundary_margin:] = 1
                pred_i = nn.functional.interpolate(
                    pred_i, size=[image_h_split, image_w_split], mode='area'
                )  # (B, num_classes, image_h_split, image_w_split)
        ...
        if depth is not None:
            ...
            pred = torch.where(mask, all_preds[-1], pred)
