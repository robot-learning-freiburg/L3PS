...
warnings.filterwarnings(
    "ignore", ".*The default behavior for interpolate/upsample with float*"
)
warnings.filterwarnings(
    "ignore",
    ".*Default upsampling behavior when mode=bicubic is changed to align_corners=False*",
)
warnings.filterwarnings(
    "ignore", ".*Only one label was provided to `remove_small_objects`*"
)
warnings.filterwarnings(
...
class SemanticFineTuner(FineTuner):
    ...
    def __init__(
        ...
        dinov2_vit_model: str,
        num_classes: int,
        output_size: Tuple[int, int],  # TODO: rename dinov2_vit_model to vit_model
        ...
        if head == "linear":
            self.head = nn.Conv2d(
                head_input_dim, num_classes, kernel_size=1, stride=1, padding=0
            )
        elif head == "cnn":
    ...
    def training_step(
        self, train_batch: Dict[str, Any], batch_idx: int
    ) -> Dict[str, Any]:
        rgb = train_batch["rgb"]
        sem = train_batch["semantic"].long()

        sem = TF.resize(
            sem, self.output_size, interpolation=T.InterpolationMode.NEAREST
        )
        pred = self(rgb)
        seg_loss = F.cross_entropy(
            pred, sem, ignore_index=self.ignore_index, reduction="none"
        )
    ...
    def predict(
        ...
        if self.test_multi_scales is None:
            ...
            pred = self.multi_scale_test_augmentation(
                rgb, apply_softmax=True, depth=depth
            )
            # (B, num_classes, H, W)

        pred = nn.functional.interpolate(
            pred, size=self.output_size, mode="bilinear", align_corners=False
        )
        pred_probs = pred
    ...
    def plot(self, rgb: np.array, pred: np.array, title: str = None):
        ...
        if self.test_save_dir is not None:
            ...
            for pred_i, rgb_i, semantic_path in zip(pred, rgb_original, semantic_paths):
                ...
                if self.test_save_vis:
                    pred_sem_i_color = self.get_dataset().class_id_to_color()[
                        pred_i, :
                    ]  # (H, W, 3)
